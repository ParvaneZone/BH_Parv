...
# =============================================================================
#  Backhaul Manager
#  Interactive helper to create / manage many Backhaul tunnels
#  Project: https://github.com/Musixal/Backhaul
#
#  Roles (as in the Backhaul README):
#    SERVER = the Iran side   (listens on the tunnel port, opens the user ports)
#    CLIENT = the foreign side (connects out to the server)
#
#  Each tunnel gets:  /etc/backhaul/<name>.toml
#                     /etc/systemd/system/backhaul-<name>.service
# =============================================================================
set -o pipefail

# ---- paths (BH_ROOT / BH_NOSYSTEMD exist only for dry-run testing) ----------
ROOT="${BH_ROOT:-}"
...
SELF_CMD="$ROOT/usr/local/bin/bhm"
...
ask_int() { # ask_int "Prompt" default min max
  local v
  while true; do
    v=$(ask "$1" "$2")
...
choose() { # choose "Prompt" default_index "value|description" ...
...
spec_local() { # local listening port (or range) of a port spec, in ufw syntax
  local s="$1" l
  if   [[ $s == *=* ]]; then l="${s%%=*}"
  elif [[ $s == *:* ]]; then l="${s%%:*}"
  else l="$s"; fi
...
get_ports() { # forwarded ports of a server tunnel, one per line
  awk '/^ports[[:space:]]*=[[:space:]]*\[/{f=1;next} f&&/^\]/{f=0} f' "$CONF_DIR/$1.toml" | tr -d ' ",'
}

set_ports() { # name ports...
  local n="$1"; shift
...
# ---- variables shared by the wizards ---------------------------------------
reset_vars() {
...
# ---- connection string (server -> client convenience) ----------------------
make_conn_string() { # host
...
install_self() {
  if [[ -f $0 ]]; then
    install -m 755 "$0" "$SELF_CMD" && ok "Installed. From now on just run: bhm"
  else
    err "Save the script to a file first, then run this option again."
  fi
}
...
main_menu() {
  ...
  while true; do
    ...
    echo "  7) Install this script as the 'bhm' command"
    echo "  0) Exit"
